...
function ingame(){
  ...
  for(let i=myList.length-1;i>=0;i--){
    myList[i].update()
    myList[i].show()
    if(myList[i] != mrSnake && mrSnake.isTouching(myList[i])){
      myList.splice(i,1)
  ...
  player.transform.position.x += (mrSnake.snakeBody[0].x-player.transform.position.x)*0.01
  player.transform.position.z += (min(mrSnake.snakeBody[0].z,350)-250-player.transform.position.z)*0.01
  if(mrSnake.hasCollided()){
    gameState = "gameover"
  }
...
function createHUD() {
  ...
  myHud.innerHTML = "Score: <span id='scoreDiv'></span><br>fps: <span id='fpsDiv'></span>"; // Insert text
  myHud.id = "hud";
